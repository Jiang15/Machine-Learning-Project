...
def tile_image(im, tilesize):
    ...
    def get_tile(tup):
        (r, c) = tup
        return torch.from_numpy(np.expand_dims(np.expand_dims(im[r:r+tilesize, c:c+tilesize],0),0))
...
def untile_image(imlist, imshape):
    ...
    """
    tilesize = imlist[0].detach().numpy().shape[2]
    offset =(tilesize)//2
    ...
    for im, tup in zip(imlist, tile_pos):
        ...
        out_im[r+rsta:r+rsto, c+csta:c+csto] = im.detach().numpy()[0,0,rsta:rsto,csta:csto]
